...
BizAPP.UI.TextEditor = {
	...
	Ig_PasteImage: function () {
		$('iframe').contents().find('body').on('paste', function (e) {
			...
			for (var i = 0; i < orgEvent.clipboardData.items.length; i++) {
				if (orgEvent.clipboardData.items[i].kind == "file" && orgEvent.clipboardData.items[i].type == "image/png") {
					...
					fileReader.onloadend = function () {
						$(getSourceElement(e)).append('<img src="' + fileReader.result + '">');
					}
		...
		$('iframe').contents().find('body')[0].addEventListener('drop', function (e) {
			var fileReader = new FileReader();
			fileReader.onloadend = function () {
				$(getSourceElement(e)).append('<img src="' + fileReader.result + '">');
			}
...
<td class="ext-upload" style="display:none" onclick="BizAPP.UI.Upload.SaveExternalLink(event)"><i class="fa fa-upload" title="Upload"></i></td><td onclick="BizAPP.UI.Upload.ShowAdvancedOptions(event)"><i class="fa fa-ellipsis-v" title="Advanced Options"></i></td>',
	...
	Init: function (aIdentifier) {
		...
		appid = JSON.parse(BizAPP.UI.currentApplication).bizappcurrentapplicationeid;
		$('.ImgCrop #photo').attr('src', 'download.asmx?aid=' + aIdentifier + '&bza-transmit=1&html.app=' + appid);
		$('.ImgCrop #preview img').attr('src', 'download.asmx?aid=' + aIdentifier + '&bza-transmit=1&html.app=' + appid);
